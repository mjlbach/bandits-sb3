    ...
    def forward(self, input_dict, state, seq_lens):
        ...
        for key, value in orig_obs.items():
            if key in ["scene_graph"]:
                ...
                value = self.feature_extractors[key]._value_branch_separate(self.feature_extractors[key]._last_flat_in)
                value_outs.append(value)
                outs.append(nn_out)

        # Concat all outputs and the non-image inputs.
        out = torch.cat(outs, dim=1)
        value_out = torch.cat(value_outs, dim=-1)
        ...
        logits, values = self.logits_layer(out), self.value_layer(value_out)
